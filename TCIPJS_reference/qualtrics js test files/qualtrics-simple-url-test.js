/**
 * SIMPLE URL TEST for Qualtrics
 * 
 * This version just tests if the task URL is accessible and shows the result.
 */

Qualtrics.SurveyEngine.addOnload(function() {
    console.log('🔍 SIMPLE URL TEST: Starting...');
        ...
        .catch(error => {
            ...
            resultsDiv.innerHTML = `
                <div style="text-align: center; padding: 50px; color: red; background-color: #f8d7da; border-radius: 5px;">
                    ...
                    <p>This means the task cannot be loaded.</p>
                </div>
            `;
    ...
    function loadIframe() {
        ...
        iframe.onload = function() {
            ...
            document.getElementById('test-status').textContent = '✅ URL accessible and iframe loaded';
            document.getElementById('test-status').style.color = 'green';
        };
        
        iframe.onerror = function() {
            console.error('🔍 Iframe failed to load');
            document.getElementById('test-status').textContent = '❌ Iframe failed to load';
            document.getElementById('test-status').style.color = 'red';
        };
